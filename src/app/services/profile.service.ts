...
updateName(fullName: string): Promise<void> 
  ...
  async updateEmail(newEmail: string, password: string): Promise<void> 
    {
      const credential: firebase.auth.AuthCredential = auth.EmailAuthProvider.credential(this.currentUser.email, password);
      try {
    ...
    async updatePassword(newPassword: string, oldPassword: string): Promise<void> {
      const credential: firebase.auth.AuthCredential = auth.EmailAuthProvider.credential(this.currentUser.email, oldPassword);
      try {
        await this.currentUser.reauthenticateWithCredential(credential);
    ...
    updateJob(fullJob: string): Promise<void> 
  {
    return this.userProfile.update({ fullJob });
  }
}